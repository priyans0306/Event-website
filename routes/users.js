...
// @desc    Update user general settings
// @route   PUT /api/users/settings/general
// @access  Private
router.put('/settings/general', protect, [
  body('firstName').optional().trim().isLength({ min: 2, max: 50 }).withMessage('First name must be between 2 and 50 characters'),
  body('lastName').optional().trim().isLength({ min: 2, max: 50 }).withMessage('Last name must be between 2 and 50 characters'),
  ...
  try {
    ...
    const { firstName, lastName, email, bio, university } = req.body;
    ...
    logger.info('User general settings updated', {
      userId: req.user._id,
      updatedFields: Object.keys(updateData)
    });
    ...
    logger.error('Update user general settings error:', error);
    res.status(500).json({
      success: false,
